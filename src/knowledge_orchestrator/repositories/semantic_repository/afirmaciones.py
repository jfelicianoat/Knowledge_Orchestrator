    ...
    def add_claim(self, note_id: int, claim: ExtractedClaim, *, source_path: Path) -> KnowledgeClaim:
        ...
        with self.database.transaction(immediate=True) as connection:
            ...
            connection.execute(
                ...
                "span_start, span_end, source_path, source_content_hash) "
                "SELECT ?, ?, ?, ?, ?, ?, ?, ? WHERE ? IS NULL ON CONFLICT DO NOTHING",
                (
                    row["claim_id"], context["capture_id"], note_id, claim.quote,
                    claim.span_start, claim.span_end, str(source_path), context['content_hash'],
    ...
    def set_manual_lock(self, claim_id: int, locked: bool) -> None:
        with self.database.transaction(immediate=True) as connection:
            if claim_in_application(connection, claim_id):
                raise ValueError('Hay una aplicación pendiente; recupérela antes de cambiar el bloqueo')
            ...
            connection.execute(
                ...
                "('CLAIM_MANUAL_LOCK_CHANGED', 'Bloqueo manual de claim modificado', ?)",
                (json.dumps({'claim_id': claim_id, 'locked': locked, 'actor': 'human:review'}),),
            )
    ...
    def find_related(self, new_claim: KnowledgeClaim, *, limit: int = 20) -> list[tuple[KnowledgeClaim, str]]:
        ...
        with closing(self.database.connect()) as connection:
            rows = connection.execute(
                ...
                "WHERE k.status = 'ACTIVE' AND k.knowledge_state IN "
                "('CURRENT','DISPUTED','UNCERTAIN','REVIEW_REQUIRED') AND n.status = 'PUBLISHED' "
                "AND k.note_id <> ? "
